      ...
      <div className="w-full max-w-md flex flex-col gap-6">
        ...
        {preview ? (
          <div className="flex flex-col gap-4">
            ...
            {analyzing ? (
              <div className="glass-card flex flex-col items-center gap-3 p-6 rounded-2xl">
                <Loader2 className="h-8 w-8 animate-spin text-primary" />
                <p className="text-sm font-medium text-muted-foreground">{t('ai.analyzing')}</p>
              </div>
            ) : (
              <div className="flex gap-3">
                <button onClick={handleUsePhoto} className="gradient-btn h-14 flex-1 rounded-2xl text-lg font-semibold">{t('imageUpload.usePhoto')}</button>
